...
class DeviceData:
    ...
    # MRS
    def display_dump_stats_json(self):
        """ Dump all the stats in a json format
        """

        for key in self.__rpc_data:
            if key in self.__rpc_data['ops']:
                self.__rpc_data[key] = self.__json_rpc_op_stats(self.__rpc_data[key])
        data = {
            'nfs': self.__nfs_data,
            'rpc': self.__rpc_data,
        }
...
def jsonstats_command(args):
    ...
    for mp in mountpoints:
        stats = DeviceData()
        stats.parse_stats(mountstats[mp]) 
        print(stats.display_dump_stats_json())
...
def main():
    ...
    json_parser = subparsers.add_parser('jsonstats',
        parents=[common_parser],
        help='Dump all the metrics in a json format.')
    json_parser.add_argument('mountpoints', nargs='*', action=ICMAction, metavar='mountpoint',
        help='Display statsistics for this mountpoint. More than one may be specified. '
            'If absent, statistics for all NFS mountpoints will be generated.')
    json_parser.set_defaults(func=jsonstats_command)
...
try:
    if __name__ == '__main__':
        ...
        elif sys.argv[1] not in ['-h', '--help', 'mountstats', 'iostat', 'nfsstat', 'jsonstats']:
            sys.argv.insert(1, 'mountstats')
        res = main()
        sys.stdout.close()
        sys.stderr.close()
